...
public class MailUtils {
  ...
  /**
   * Send a message to the transport configured via the provided properties.
   * Encapsulates the MIME multipart construction with the provided HTML
   * and plain text forms.
   *
   * One of either plainText or htmlText (or both) are required. If both are null
   * or empty we throw an IllegalArgumentException.
   *
   * In the case of only having htmlText, plainText is extracted from the body
   * and wrapped in &lt;pre&gt; tags to create the multi-part.
   *
   * @param props
   * @param to
   * @param from
   ...
   */
    ...
    Preconditions.checkArgument(StringUtils.isNotBlank(htmlText) || StringUtils.isNotBlank(plainText),
		...
		try {
			...
			String username = props.getProperty(MAIL_PROP_PREFIX + protocol
					+ ".username");
			String password = props.getProperty(MAIL_PROP_PREFIX + protocol
					+ ".password");
			...
			if (plainText != null) {
				...
				plainPart.setHeader(CONTENT_TYPE,
			...
			if (htmlText != null) {
				...
				htmlPart.setHeader(CONTENT_TYPE,
  ...
  /**
   * Delegates to {@link #sendMail(java.util.Properties, String, String, String, String, String)}
   * with null as the 'plainText' argument to such.
   *
   * @param props
   * @param to
   * @param from
