...
public class ConversionUtils {
	...
	public static String string(ByteBuffer bytes) {
		...
		return string(bytes.array(), bytes.arrayOffset() + bytes.position(),
				bytes.remaining(), UTF8_ENCODING);
	}
	...
	public static Object object(Class<?> type, byte[] bytes) {
		...
		try {
			...
			logger.error(
					"Unable to get object from bytes for type "
							+ type.getName(), e);
		}
		return null;
	}
	 ...
	 * @return
	 */
	public static ByteBuffer appendToByteBuffer(ByteBuffer bb, byte[] bytes,
			int len) {
		if (len > bytes.length) {
			int pos = bb.position();
			bb.put(bytes);
		...
		for (Map.Entry<String, Object> entry : values.entrySet()) {
			if (types.containsKey(entry.getKey())) {
				values.put(entry.getKey(),
						coerce(types.get(entry.getKey()), entry.getValue()));
			}
		}
		return values;
	}
