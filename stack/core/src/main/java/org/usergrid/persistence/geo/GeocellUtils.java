...
public final class GeocellUtils {
   ...
   * given Northeast geocell to the given Southwest geocell. Assumes the
   * Northeast geocell is actually Northeast of Southwest geocell.
   * 
   * @param cellNE
   *          : The Northeast geocell string.
   * @param cellSW
   ...
   * 
   * @param cell
   *          : The geocell string for which to calculate adjacent/neighboring
   *          cells.
   * @return A list of 8 geocell strings and/or None values indicating adjacent
   *         cells.
   */
   ...
   * @param dir
   *          : An (x, y) tuple indicating direction, where x and y can be -1,
   *          0, or 1. -1 corresponds to West for x and South for y, and 1
   *          corresponds to East for x and North for y. Available helper
   ...
   * 
   * If the point is inside the cell, the shortest distance is always to a
   * 'edge' of the cell rectangle. If the point is outside the cell, the
   * shortest distance will be to either a 'edge' or 'corner' of the cell
   * rectangle.
   * 
   * @param cell
   * @param point
   * @return The shortest distance from the point to the geocell's rectangle, in
   *         meters.
   */
  public static double pointDistance(String cell, Point point) {
    ...
    if (betweenWE) {
      if (betweenNS) {
        // Inside the geocell.
        return Math.min(
            Math.min(distance(point, new Point(bbox.getSouth(), point.getLon())),
      ...
      if (betweenNS) {
        ...
        return Math.min(
            Math.min(distance(point, new Point(bbox.getSouth(), bbox.getEast())),
   ...
   * 
   * @param cell_
   *          : The geocell string whose boundaries are to be computed.
   * @return A geotypes.Box corresponding to the rectangular boundaries of the
   *         geocell.
   */
   ...
   * 
   * @param pos
   * @return Returns the geocell character in the 4x4 alphabet grid at pos. (x,
   *         y).
   */
  ...
  /**
   * Returns the edges of the rectangular region containing all of the given
   * geocells, sorted by distance from the given point, along with the actual
   * distances from the point to these edges.
   * 
   * @param cells
   *          : The cells (should be adjacent) defining the rectangular region
   *          whose edge distances are requested.
   * @param point
   *          : The point that should determine the edge sort order.
   * @return A list of (direction, distance) tuples, where direction is the edge
   *         and distance is the distance from the point to that edge. A
   *         direction value of (0,-1), for example, corresponds to the South
   *         edge of the rectangular region containing all of the given
   *         geocells.
   * 
   *         TODO(romannurik): Assert that lat,lon are actually inside the
   *         geocell.
   */
