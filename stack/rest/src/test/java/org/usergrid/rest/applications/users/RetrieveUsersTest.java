...
public class RetrieveUsersTest extends AbstractRestIT {
  private static Logger log = LoggerFactory.getLogger(RetrieveUsersTest.class);
  ...
  @Test // USERGRID-1727
  public void userEntityMetadataHasRoles() {
	CustomCollection users = context.collection("users");
    ...
    JsonNode entity = response.findValues("entities").get(0);
    JsonNode sets = entity.path("metadata").path("sets");
	Assert.assertNotNull(sets.path("roles"));
  }
}