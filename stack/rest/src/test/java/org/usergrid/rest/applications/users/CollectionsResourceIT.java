...
public class CollectionsResourceIT extends AbstractRestIT {
  ...
  /**
   * emails with "me" in them are causing errors. Test we can post to a colleciton after creating a
  ...
  public void permissionWithMeInString() throws Exception {
    ...
    createUser("sumeet.agarwal@usergrid.com", 
            "sumeet.agarwal@usergrid.com", "secret", "Sumeet Agarwal");
    ...
    JsonNode posted = resource().path(path)
    ...
    posted = resource()
            .path("/test-organization/test-app/nestprofiles")
            .queryParam("access_token", token)
            .accept(MediaType.APPLICATION_JSON)
  ...
  public void stringWithSpaces() {
    Map<String, String> payload = 
            hashMap("summaryOverview", "My Summary").map("caltype", "personal");
