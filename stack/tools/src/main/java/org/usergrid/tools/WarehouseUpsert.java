 ...
 * is distributed on an "AS IS" BASIS, WITHOUT DITIONS OF ANY KIND, either express
...
public class WarehouseUpsert extends ExportingToolBase {
  ...
  public void runTool(CommandLine line) throws Exception {
    ...
    String constr = String.format("jdbc:postgresql://%s:%s/%s?user=%s&password=%s",
    ...
    String upsertUpdate = String.format(
      "UPDATE %s SET id = s.id FROM %s s WHERE %s.created = s.created ",
    ...
    String upsertInsert = String.format(
      "INSERT INTO %s SELECT s.* FROM %s s LEFT JOIN %s n ON s.id = n.id WHERE n.id IS NULL",
      mainTableName, stagingTableName, mainTableName);
