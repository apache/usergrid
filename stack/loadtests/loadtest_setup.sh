...
if [[ $check == "" ]]
then 
sed -i.bak 's/JAVA_OPTS="/JAVA_OPTS="-DnumUsers=${GATLING_NUMUSERS} -DrampTime=${GATLING_RAMPTIME} -Dduration=${GATLING_DURATION} -DnumEntities=${GATLING_NUMENTITIES} -Dbaseurl=${GATLING_BASE_URL} -Dorg=${GATLING_ORG} -Dapp=${GATLING_APP} /g' gatling/bin/gatling.sh
fi
