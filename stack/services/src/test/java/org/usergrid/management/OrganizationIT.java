...
public class OrganizationIT
{
	...
	public void testCreateOrganization() throws Exception {
		...
		OrganizationInfo organization = setup.getMgmtSvc().createOrganization(
                "OrganizationIT", user, false);
		assertNotNull(organization);

		Map<UUID, String> userOrganizations = setup.getMgmtSvc()
				.getOrganizationsForAdminUser(user.getUuid());
		assertEquals("wrong number of organizations", 1,
				userOrganizations.size());

		List<UserInfo> users = setup.getMgmtSvc()
		...
		OrganizationInfo organization2 = setup.getMgmtSvc()
				.getOrganizationForApplication(applicationId);
		assertNotNull(organization2);
		assertEquals("wrong organization name", "OrganizationIT",
				organization2.getName());

		boolean verified = setup.getMgmtSvc().verifyAdminUserPassword(user.getUuid(),
                "test");
		assertTrue(verified);

        setup.getMgmtSvc().activateOrganization(organization2);

		UserInfo u = setup.getMgmtSvc().verifyAdminUserPasswordCredentials(user
		...
		AuthPrincipalInfo principal = ((ManagementServiceImpl) setup.getMgmtSvc())
  ...
  public void testPasswordHistoryCheck() throws Exception {
    ...
    UserInfo user = setup.getMgmtSvc().createAdminUser("edanuff3", "Ed Anuff", "ed2@anuff.com2", passwords[0], true, false);
    assertNotNull(user);
